...
import {
  ...
  UpdateBehaviour, ContainerUpdateBehaviour,
  Text,
  Rect,
  entity,
  Sound,
...
const sounds = {
  theme: new Sound(theme, { volume: 0.05, loop: true }),
};
...
export class GameScreen extends Container {
  ...
  constructor(
    ...
    Events.on(this.engine, 'collisionStart', e => {
      const body = e.pairs
        .reduce((ac, el) => [...ac, el.bodyA, el.bodyB], [])
        .find(b => b === penguin.body);
      if (body && body.speed > 5) {
        penguin.collide(body.speed);
      }
    });
